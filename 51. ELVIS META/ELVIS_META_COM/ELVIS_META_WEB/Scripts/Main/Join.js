...
$(function () {
    ...
    $(".btns.next").on("click", function () {
        if ($('.join-status__info.status1').hasClass('on')) {
            if ((!$("#SHP").hasClass("on")) && (!$("#EXE").hasClass("on"))) {
                _fnAlertMsg("고객님의 유형을 선택해 주세요.");
                return false;
            }
            ...
            if ($("#SHP").hasClass("on")) {
                $(".RegistExecut").hide();
                $(".shipper .joinServ").each(function () {
                    if ($(this).hasClass("on")) {
                        if (_fnToNull($(this).find("p").text()) == "서식") {
                            objData.DOCU = "Y";
                        } else if (_fnToNull($(this).find("p").text()) == "견적") {
                            objData.QUOT= "Y";
                        } else if (_fnToNull($(this).find("p").text()) == "수출") {
                            objData.EXPORT = "Y";
                        } else if (_fnToNull($(this).find("p").text()) == "수입") {
                            objData.IMPORT = "Y";
                        }
                    }
                });
            } else if ($("#EXE").hasClass("on")) {
                $(".execution .joinServ").each(function () {
                    if ($(this).hasClass("on")) {
                        if (_fnToNull($(this).find("p").text()) == "견적") {
                            objData.QUOT = "Y";
                        } else if (_fnToNull($(this).find("p").text()) == "해운수출") {
                            objData.SEA_EX = "Y";
                        } else if (_fnToNull($(this).find("p").text()) == "해운수입") {
                            objData.SEA_IM = "Y";
                        } else if (_fnToNull($(this).find("p").text()) == "항공수출") {
                            objData.AIR_EX = "Y";
                        } else if (_fnToNull($(this).find("p").text()) == "항공수입") {
                            objData.AIR_IM = "Y";
                        }
                    }
                });
...
function fnChoiceService() {
    ...
    $("#choiceSvc").append(vHtml);
}

function fnCheckRegist() {
    $("#LastEmail").val(_fnToNull($("#RES_EMAIL").val()));
    $("#LastName").val(_fnToNull($("#RES_NAME").val()));
    $("#LastTel").val(_fnToNull($("#RES_TEL").val()));
    $("#LastCustNm").val(_fnToNull($("#CUST_NM").val()));
    $("#LastCustCrn").val(_fnToNull($("#RES_CRN").val()));
    $("#LastFile").val(_fnToNull($("#Elvis_Crn_FileName").val()));
}
...
function fnIsWarningMSG() {
    ...
    if (vEmail_Hidden == "true" && vPW1_Hidden == "true" && vPW2_Hidden == "true" && NAME_Hidden == "true" && TEL_Hidden == "true" && CustName_Hidden == "true") {
        return true;
    } else {
        return false;
    }
    ...
    try {
        if (fnValidation()) {
           ...
           // var objJsonData = new Object();
            ...
            objData.USR_ID = _fnToNull($("#RES_EMAIL").val()); //이메일
            objData.PSWD = _fnToNull($("#RES_PWD").val()); //비밀번호
            objData.USR_NM = _fnToNull($("#RES_NAME").val()); //이름
            objData.HP_NO = _fnToNull($("#RES_TEL").val().replace(/-/gi, "")); //연락처
            objData.LOC_NM = _fnToNull($("#CUST_NM").val()); //회사명
            objData.DOMAIN = _fnToNull($("#Domain").val()); //도메인
            objData.CRN = _fnToNull($("#RES_CRN").val().replace(/-/gi, ""));
...
function fnValidation() {
    ...
    if (_fnToNull($("#CUST_NM").val().replace(/ /gi, "")) == "") {
        _fnAlertMsg("사업자 명을 입력 해 주세요.", "CUST_NM");
        return false;
    }
    ...
    if ($("#EXE").hasClass("on")) {
        if (_fnToNull($("#Domain").val().replace(/ /gi, "")) == "") {
            _fnAlertMsg("엘비스 도메인을 입력 해 주세요.", "Domain");
            return false;
        }

        if ($("input[name=custfiles]")[0].files.length == 0) {
            _fnAlertMsg("업체 소개 자료를 선택해 주세요.", "CompanyIntro_FileName");
            return false;
        }
    }
