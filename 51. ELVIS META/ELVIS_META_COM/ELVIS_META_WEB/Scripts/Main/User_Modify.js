   ...
   if (_fnToNull($("#Session_USR_ID").val()) == "") {
    ...
    try {
        ...
        if (JSON.parse(result).Result[0].trxCode == "Y") {
            ...
            objLogin_Info.USR_TYPE = JSON.parse(result).Table1[0].USR_TYPE;
            objLogin_Info.DOCU = JSON.parse(result).Table1[0].DOCU
            objLogin_Info.QUOT = JSON.parse(result).Table1[0].QUOT;
            objLogin_Info.EXPORT = JSON.parse(result).Table1[0].EXPORT;
            objLogin_Info.IMPORT = JSON.parse(result).Table1[0].IMPORT;
            objLogin_Info.SEA_EX = JSON.parse(result).Table1[0].SEA_EX;
            objLogin_Info.SEA_IM = JSON.parse(result).Table1[0].SEA_IM;
            objLogin_Info.AIR_EX = JSON.parse(result).Table1[0].AIR_EX;
            objLogin_Info.AIR_IM = JSON.parse(result).Table1[0].AIR_IM;
        ...
        var vHtml = "";
        //$("#NOW_EMAIL").val(atob(objLogin_Info.USR_ID));
        $("#NOW_EMAIL").val(objLogin_Info.USR_ID);
        $("#NOW_LOC_NM").val(objLogin_Info.USR_NM);
        $("#NOW_CRN").val(objLogin_Info.CRN);
        $("#NOW_NAME").val(objLogin_Info.LOC_NM);
        $("#NOW_TEL").val(objLogin_Info.HP_NO);
        $("#NOW_CRN").text(objLogin_Info.CRN);
        if (objLogin_Info.USR_TYPE == "S") {
            vHtml += " <div class='list-info join_sort'>";
            vHtml += "    <p>화주</p>";
            vHtml += "  </div>";
            if (objLogin_Info.DOCU == "Y") {
                $("#DOCU_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>서식</p>";
                vHtml += "  </div>";
            }
            if (objLogin_Info.QUOT == "Y") {
                $("#QUOT_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>견적</p>";
                vHtml += "  </div>";
            }
            if (objLogin_Info.EXPORT == "Y") {
                $("#EX_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>수출</p>";
                vHtml += "  </div>";
            }
            if (objLogin_Info.IMPORT == "Y") {
                $("#IM_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>수입</p>";
                vHtml += "  </div>";
            ...
            vHtml += "  </div>";
            if (objLogin_Info.QUOT == "Y") {
                $("#F_QUOT_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>견적</p>";
                vHtml += "  </div>";
            }
            if (objLogin_Info.SEA_EX == "Y") {
                $("#SEA_EX_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>해운수출</p>";
                vHtml += "  </div>";
            }
            if (objLogin_Info.SEA_IM == "Y") {
                $("#SEA_IM_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>해운수입</p>";
                vHtml += "  </div>";
            }
            if (objLogin_Info.AIR_EX == "Y") {
                $("#AIR_EX_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>항공수출</p>";
                vHtml += "  </div>";
            }
            if (objLogin_Info.AIR_IM == "Y") {
                $("#AIR_IM_YN").addClass("on");
                vHtml += " <div class='list-info'>";
                vHtml += "    <p>항공수입</p>";
                vHtml += "  </div>";
            }
        }
        $("#USR_ITEM").append(vHtml);
...
$(document).on("click", "#btn_Regester", function (e) {
    var objData = new Object();

    objData.USR_ID = _fnToNull($("#NOW_EMAIL").val());
    objData.LOC_NM = _fnToNull($("#NOW_LOC_NM").val());
    objData.TEL_NO = _fnToNull($("#NOW_TEL").val());
    objData.CUST_NM = _fnToNull($("#NOW_NAME").val());
    objData.DOMAIN = _fnToNull($("#domain").val());

    _fnAlertMsg("수정요청 되었습니다.<br>(익월 반영 됩니다.)");
    $("#alert_close").click(function () {
        window.location = window.origin + "/Info/Dashboard";
    });

});

$(document).on("click", "#btn_SaveService", function (e) {
    const serviceIds = ["DOCU_YN", "QUOT_YN", "EX_YN", "IM_YN"];
    const objData = {};

    // id별 Y/N값 obj에 넣기
    serviceIds.forEach(function (id) {
        if ($("#" + id).hasClass("on")) objData[id] = "Y";
        else objData[id] = "N";
    });

    // 한 가지 이상 서비스를 이용
    if (Object.values(objData).every(value => value === 'N')){
        _fnAlertMsg("서비스를 최소 한 가지 이상 선택해주세요.");
    } else {
        objData.USR_ID = _fnToNull($("#NOW_EMAIL").val());

        $.ajax({
            type: "POST",
            url: "/Main/UserServiceInfoUpdateApi",
            async: true,
            dataType: "json",
            //data: callObj,
            data: { "vJsonData": _fnMakeJson(objData) },
            success: function (result) {
                _fnAlertMsg("수정되었습니다.<br>보안을 위해 로그아웃됩니다.")
                $("#alert_close").click(function () {
                    _fnLogout();
                })
...
$(document).on("click", "#btn_confirm", function (e) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $(".alert_cont .inner").html("탈퇴처리 되었습니다");
            layerPopup('#alertBye');

            $("#alert_bye").click(function () {
                _fnLogout();
            });
        }, error: function (xhr, status, error) {
            $("#ProgressBar_Loading").hide(); //프로그래스 바
