...
$(".btns.next").on("click", function () {
    if (ChkValidation()) {
        ...
        if ($("#doc_file1")[0].files.length == 0 && $("#doc_file2")[0].files.length == 0) {
            if ($(this).closest(".req-info__seq").index() == "1") { // 두칸 앞으로
                chkFile = true;
                ...
                if (!$(this).hasClass("request")) {
                    ...
                    $(".req-status__info").eq(statNum - 1).addClass("passed");
                    $(".req-status__info").eq(statNum2 - 2).addClass("passed");
                    $(".req-status__info").eq(statNum2 - 1).addClass("on");
                }
                ...
                if (!$(this).hasClass("request")) {
                    ...
                    //$(".req-status__info:nth-child(" + statNum + ")").addClass("passed");
                    //$(".req-status__info:nth-child(" + statNum2 + ")").addClass("on");
                    $(".req-status__info").eq(statNum - 2).addClass("passed");
                    $(".req-status__info").eq(statNum-1).addClass("passed");
                    $(".req-status__info").eq(statNum2-1).addClass("on");
                }
            }
...
$(".btns.prev").on("click", function () {
    ...
    if ($("#doc_file1")[0].files.length == 0 && $("#doc_file2")[0].files.length == 0) {
        var $seq = $(this).closest(".req-info__seq");
        if ($(this).closest(".req-info__seq").index() == "3") { //두칸 뒤로
            var statNum = $(this).closest(".req-info__seq").index() ;
            var statNum2 = $(this).closest(".req-info__seq").index()-1;
            $seq.hide();
            ...
            $(".req-status__info").eq(statNum - 2).removeClass("passed"); //돌아갈 곳 pssed 지우기
            //$(".req-status__info").eq(statNum - 2).addClass("on"); //돌아갈 곳 on 지우기
            $(".req-status__info").eq(statNum2 - 2).removeClass("passed"); //돌아갈 곳 pssed 지우기
            $(".req-status__info").eq(statNum2 - 2).addClass("on"); //돌아갈 곳 on 지우기
            ...
            $(".req-status__info").eq(statNum - 1).removeClass("passed"); //돌아갈 곳 pssed 지우기
            $(".req-status__info").eq(statNum2 - 1).removeClass("passed"); //돌아갈 곳 on 지우기
            $(".req-status__info").eq(statNum2 - 1).addClass("on"); //돌아갈 곳 on 지우기
        }
    } else { // 한칸 뒤로
        var $seq = $(this).closest(".req-info__seq");
        var statNum = $(this).closest(".req-info__seq").index();
...
$(document).keyup(function (e) {
    if (e.keyCode == 13) {
        ...
        if ($(e.target).attr('data-index') != undefined) {
            ...
            if (parseFloat(vIndex) == 2 || parseFloat(vIndex) == 4) { // 날짜 인풋박스 일때
...
function fnSearchForwarder() {
    ...
    if (JSON.parse(rtnVal).Result[0]["trxCode"] == "Y") {
        ...
        $("#FRD_LIST1").append(LvHtml);
        $("#FRD_LIST2").append(RvHtml);
    } else {
        $("#FRD_LIST1").empty();
        $("#FRD_LIST2").empty();
    }
...
function fnRequestData() {
    ...
    if (_fnToNull($("#input_POLCD").val()).substring(0, 2) != "KR") {
...
// fnSearchForwarder - > "포워더 조회" 검색 함수
